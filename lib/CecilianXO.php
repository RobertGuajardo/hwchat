...
class CecilianXO
{
    ...
    public function search(array $criteria = [], int $limit = 5): array
    {
        ...
        $fetchAll = !empty($criteria['fetch_all']) || !empty($criteria['listing_type']) || !empty($criteria['lot_type']) || !empty($criteria['garage_min']) || !empty($criteria['stories']) || !empty($criteria['baths_min']) || !empty($criteria['sqft_min']) || !empty($criteria['sqft_max']);
    ...
    private function applySort(array &$properties, string $sortBy): void
    {
        ...
        usort($properties, function ($a, $b) use ($sortBy) {
            switch ($sortBy) {
                ...
                case 'status_first':
                    // Move-in ready first, then construction, then available, then price
    ...
    private function formatForChat(array $prop): array
    {
        ...
        $formatted = [
            ...
            'listing_type'  => $this->resolveListingType($prop),
            'lot_type'      => $this->resolveLotType($prop),
